export default class ExportService {
    ...
    amoCRM() {
        return this.$api.get('/export_to_amo')
            .then((response) => {
                const file = new this.Blob([response], { type: 'application/text' });
    ...
    poll(id) {
        return this.$api.get(`polls/${id}/export_to_xlsx`)
          .then((response) => {
              ...
              const blob = new Blob([s2ab(atob(response))], { type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
